...
class LatentDiffusion(DDPM):
    ...
    def p_losses(self, x_start, mask,cond, t, noise=None, **kwargs):
        noise = default(noise, lambda: torch.randn_like(x_start))
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        model_output = self.apply_model(x_noisy,mask, t, cond, **kwargs)
    ...
    def p_losses_origin(self, x_start, cond, t, noise=None, **kwargs):
        noise = default(noise, lambda: torch.randn_like(x_start))
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(self, x,mask, t, c_concat: list = None, c_crossattn: list = None, c_adm=None, **kwargs):
        if self.conditioning_key is None:
            ...
            out = self.diffusion_model(x, mask,t, context=cc, **kwargs)
